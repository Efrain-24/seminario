...
use App\Models\InventarioExisten        // Verificar que el item seleccionado es realmente un alimento y hay stock suficiente
        $inventarioItem = InventarioItem::find($validated['inventario_item_id']);
        if (!$inventarioItem || $inventarioItem->tipo !== 'alimento') {
            return back()->withErrors(['inventario_item_id' => 'El item seleccionado debe ser un alimento.'])->withInput();
        }

        // Buscar si existe un TipoAlimento asociado a este InventarioItem
...
class AlimentacionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Validar que el item de inventario sea un alimento
        $inventarioItem = InventarioItem::find($validated['inventario_item_id']);
        if (!$inventarioItem || $inventarioItem->tipo !== 'alimento') {
            return back()->withErrors(['inventario_item_id' => 'El item seleccionado debe ser un alimento.'])->withInput();
        }

        // Validar que haya suficiente stock en la bodega
        ...
        if (!$existencia || $existencia->stock_actual < $validated['cantidad_kg']) {
            return back()->withErrors(['cantidad_kg' => 'No hay suficiente stock disponible en la bodega seleccionada.'])->withInput();
        }
    ...
    public function edit(Alimentacion $alimentacion)
    {
        ...
        // Preparar datos de existencias por bodega para JavaScript
        $existenciasPorBodega = [];
        foreach ($bodegas as $bodega) {
            $existenciasPorBodega[$bodega->id] = [];
            ...
            $existencias = InventarioExistencia::where('bodega_id', $bodega->id)
                ->where('stock_actual', '>', 0)
                ->whereHas('item', function($query) {
                    $query->where('tipo', 'alimento'); // Solo items de tipo alimento
                })
        ...
        return view('alimentacion.edit', compact('alimentacion', 'lotes', 'tiposAlimento', 'bodegas', 'existenciasPorBodega'));
    }
    ...
    public function update(Request $request, Alimentacion $alimentacion)
    {
        ...
        // Verificar que el item seleccionado es realmente un alimento y hay stock suficiente
        $inventarioItem = InventarioItem::find($validated['inventario_item_id']);
        if (!$inventarioItem || $inventarioItem->tipo !== 'alimento') {
