<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-900 dark:text-gray-100">Editar ítem</h2>
    </x-slot>

    <div class="py-8 max-w-xl mx-auto px-4">
        <div class="bg-white dark:bg-gray-800 shadow rounded border border-gray-200 dark:border-gray-700 p-4">
            <form method="POST" action="{{ route('produccion.inventario.items.update', $item) }}" class="grid gap-4">
                @csrf @method('PUT')
                @include('inventario.items._form', ['item' => $item])
