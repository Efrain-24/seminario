<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-900 dark:text-gray-100">Editar Acción Correctiva</h2>
    </x-slot>
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <form action="{{ route('acciones_correctivas.update', $accion) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="titulo" id="titulo" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2" value="{{ old('titulo', $accion->titulo) }}" required>
                    @error('titulo')
                ...
                <div>
                    ...
                    <textarea name="descripcion" id="descripcion" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2" required>{{ old('descripcion', $accion->descripcion) }}</textarea>
                    @error('descripcion')
                        <div class="text-red-600 dark:text-red-400 text-sm">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <select name="user_id" id="user_id" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2" required>
                        ...
                        @isset($usuarios)
                            @foreach($usuarios as $usuario)
                                <option value="{{ $usuario->id }}" {{ old('user_id', $accion->user_id) == $usuario->id ? 'selected' : '' }}>{{ $usuario->name }}</option>
                            @endforeach
                        @endisset
                    </select>
                    @error('user_id')
                ...
                <div>
                    ...
                    <input type="date" name="fecha_limite" id="fecha_limite" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2" value="{{ old('fecha_limite', $accion->fecha_limite) }}">
                    @error('fecha_limite')
                        <div class="text-red-600 dark:text-red-400 text-sm">{{ $message }}</div>
                    @enderror
                </div>
                <div>
                    ...
                    <select name="estado" id="estado" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2" required>
                        <option value="pendiente" {{ old('estado', $accion->estado) == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                        <option value="en_progreso" {{ old('estado', $accion->estado) == 'en_progreso' ? 'selected' : '' }}>En progreso</option>
                        <option value="completada" {{ old('estado', $accion->estado) == 'completada' ? 'selected' : '' }}>Completada</option>
                        <option value="cancelada" {{ old('estado', $accion->estado) == 'cancelada' ? 'selected' : '' }}>Cancelada</option>
                    </select>
                    @error('estado')
                        <div class="text-red-600 dark:text-red-400 text-sm">{{ $message }}</div>
                    @enderror
