    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div class="flex items-center">
                <a href="{{ route('produccion.panel') }}" 
                   class="mr-4 text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200">
                                    ...
                                    @foreach($lotes as $lote)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors duration-150"
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                {{ $lote->codigo_lote ?? 'N/D' }}
                                            </td>

                                            <!-- Especie -->
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300">
                                                    {{ ucfirst($lote->especie ?? 'Desconocida') }}
                                                </span>
                                            </td>

                                            <!-- Unidad -->
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                {{ optional($lote->unidadProduccion)->nombre ?? 'Sin unidad' }}
                                            </td>
                                            ...
                                            <!-- Fecha -->
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                {{ optional($lote->fecha_inicio)->format('d/m/Y') ?? 'Sin fecha' }}
                                            </td>
