        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <form action="{{ route('limpieza.update', $limpieza) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="area" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2" value="{{ $limpieza->area }}" required>
                </div>
                <div>
                    <label for="descripcion" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Descripción</label>
                    <textarea name="descripcion" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2">{{ $limpieza->descripcion }}</textarea>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="protocolo_sanidad_id" class="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-2" required>
                        @foreach($protocolos as $protocolo)
                            <option value="{{ $protocolo->id }}" @if($limpieza->protocolo_sanidad_id == $protocolo->id) selected @endif>{{ $protocolo->nombre }}</option>
                        @endforeach
                    </select>
                </div>
